...
const filesToBeDeleted = [
  ...
  'doc/workflows/update-from-fs-agent.md',
];

const templateRepo = 'https://github.com/rljson/fs-agent.git';
const localRepoPath = path.resolve(__dirname, '../../fs-agent');
...
function replaceFsAgent() {
  ...
  for (const file of replaceFiles) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      content = content.replace(/fs-agent/g, projectName);
